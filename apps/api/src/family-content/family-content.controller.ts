...
export class FamilyContentController {
  ...
  @Post('bindings/:bindingId/publish') async publish(@Req() req: Request, @Param('bindingId') id: string, @Body() input: { previewConfirmed?: boolean }) { const parent = await this.sessions.requireParent(req); if (input.previewConfirmed !== true) throw new BadRequestException('发布前必须确认孩子页面预览'); return this.content.publish(parent.id, id) }
