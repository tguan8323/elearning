...
export function StartLesson({
  ...
  const currentContent =
    step === null
      ? ""
      : lessonTarget.parentScript[
          Math.min(step, lessonTarget.parentScript.length - 1)
        ];
  ...
  function advance() {
    ...
    cast(
      lessonTarget.parentScript[
        Math.min(nextStep, lessonTarget.parentScript.length - 1)
      ] ?? "",
    );
  }
  ...
  async function start() {
    ...
    const offlineTarget = offlinePackage?.targets.find(
    ...
    try {
      response = await fetch("/api/learning/sessions", {
        method: "POST",
        credentials: "include",
    ...
    if (response?.ok) {
      ...
      publishCastContent({
        sessionId: body.id,
        text: activeTarget.parentScript[0] ?? "",
      });
    } else {
  ...
  async function observe(
    outcome: "independent" | "prompted" | "not_observed" | "declined",
  ) {
    const observation = {
      ...
      promptLevel:
        outcome === "independent"
          ...
          : outcome === "prompted"
            ? "gesture"
            : "not_applicable",
      effectivePrompt: effectivePrompt.trim() || undefined,
      materialVariant: `lesson-stage-${step ?? 4}`,
      interestLevel,
      fatigueLevel,
      discomfort,
    ...
    try {
      ...
      await saveRecordAndQueue(
        {
          recordId: observation.clientId,
          version: 0,
          deleted: false,
          payload: observation,
        },
        createOperation({
          kind: "upsert-observation",
          recordId: observation.clientId,
          baseVersion: 0,
          payload: observation,
        }),
    ...
    try {
      ...
      await saveRecordAndQueue(
        ...
        createOperation({
          ...
          recordId: sessionId,
          baseVersion: 0,
          payload: { clientId: sessionId, targetId: target.id, status },
        }),
      );
    ...
    <section className="lessonStep" aria-live="polite">
      ...
      <div className="lessonActions">
          ...
          <div className="observationChoices">
            ...
            <label>
              <input
                type="checkbox"
                checked={discomfort}
                onChange={(event) => setDiscomfort(event.target.checked)}
              />{" "}
              感觉不适
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) => setEffectivePrompt(event.target.value)}
                maxLength={120}
                placeholder="可选"
              />
            </label>
            ...
            <button onClick={() => void observe("independent")}>
              独立完成
            </button>
            <button onClick={() => void observe("prompted")}>提示后完成</button>
            <button onClick={() => void observe("not_observed")}>
              未观察到
            </button>
        ...
        <button
          ...
          onClick={() => void finish("ENDED_EARLY")}
        >
          提前结束
        </button>
      </div>
