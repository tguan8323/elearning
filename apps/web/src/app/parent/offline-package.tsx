...
export function OfflinePackage() {
  ...
  function activate() { if (!manifest) return; navigator.serviceWorker.controller?.postMessage({ type: 'ACTIVATE_PACKAGE', urls: ['/learn'] }); window.localStorage.setItem('family-english-package', manifest.version); setActive(true); setMessage('离线包已激活；敏感操作仍需联网。') }
  return <section className="summary" aria-labelledby="offline-package-title"><h2 id="offline-package-title">离线教学包</h2><p>只包含课程核心内容和已发布的家庭内容，不包含密码、PIN 或敏感资料。</p>{manifest && <><p>版本 {manifest.version} · {manifest.sizeBytes} bytes · SHA-256 {manifest.checksum}</p><button type="button" onClick={download} disabled={!manifest}>下载离线包</button><button type="button" onClick={activate} disabled={!manifest || active}>{active ? '已激活到本机' : '激活离线包'}</button></>}<button type="button" onClick={() => void prepare()}>检查并准备离线包</button>{message && <p role="status">{message}</p>}</section>
}
