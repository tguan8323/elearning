...
export function FamilyContentManager({ items }: { items: Item[] }) {
  ...
  async function reviewAudio(versionId: string, reviewState: 'APPROVED' | 'REJECTED') { const response = await fetch(`/api/family-content/versions/${versionId}/audio-review`, { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ reviewState }) }); if (response.ok) { setStatus(reviewState === 'APPROVED' ? '音频已审核通过。' : '音频已标记为不通过。'); router.refresh() } else setError('音频审核失败，请稍后重试。') }
  const [preview, setPreview] = useState<{ versionId: string; slotId: string } | null>(null)
  async function lifecycle(versionId: string, slotId: string, previewConfirmed = false) { setError(''); if (!previewConfirmed) { setPreview({ versionId, slotId }); return }; const binding = await fetch(`/api/family-content/versions/${versionId}/bind`, { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ slotId }) }); if (!binding.ok) { setError('绑定失败，请检查插槽兼容性。'); return }; const result = await binding.json() as { id: string }; const published = await fetch(`/api/family-content/bindings/${result.id}/publish`, { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ previewConfirmed: true }) }); if (!published.ok) { setError('绑定成功，但发布失败，请稍后重试。'); return }; setPreview(null); setStatus('内容已明确发布给孩子。'); router.refresh() }
  ...
  return <section className="privateContent" aria-labelledby="private-content-title">
    ...
    <section className="contentCatalog"><h3>已上传内容</h3>{preview ? <aside className="card" aria-label="孩子页面预览"><h3>孩子页面预览</h3><p>请检查孩子页面中的低刺激显示、语言负担和媒体播放。</p><iframe title="孩子页面一致预览" src={`/api/family-content/versions/${preview.versionId}/preview`} style={{ width: '100%', minHeight: 180, border: 0 }} /><div className="lessonActions"><button type="button" onClick={() => void lifecycle(preview.versionId, preview.slotId, true)}>确认预览并发布</button><button type="button" className="secondaryButton" onClick={() => setPreview(null)}>取消</button></div></aside> : null}{items.length ? items.map(item => <article className="contentSnapshot" key={item.id}><h4>{item.title}</h4><p>{item.mediaType ?? item.contentType ?? '未分类'} · {item.source} · {item.purpose ?? item.description ?? '未填写用途'}</p>{item.versions?.map(v => <div key={v.id}><p className="formHint">{v.fileName} · {v.uploadState}</p><a href={`/api/family-content/versions/${v.id}/preview`} target="_blank" rel="noreferrer">预览文件</a>{v.mimeType.startsWith('audio/') && <span><button type="button" onClick={() => void reviewAudio(v.id, 'APPROVED')}>审核通过（美式英语）</button><button type="button" onClick={() => void reviewAudio(v.id, 'REJECTED')}>审核不通过</button>{v.audioReview ? <small> 审核：{v.audioReview.reviewState} · {v.audioReview.accent}</small> : null}</span>}{v.bindings?.flatMap(binding => binding.publications ?? []).filter(publication => publication.state === 'PUBLISHED').map(publication => <button key={publication.id} type="button" onClick={() => { if (window.confirm('确认从孩子页面撤回此内容？')) void withdraw(publication.id) }}>撤回发布</button>)}{v.uploadState === 'UPLOADED' && <div><label htmlFor={`slot-${v.id}`}>发布插槽</label><select id={`slot-${v.id}`} defaultValue="" onFocus={() => { if (!slots.length) void loadSlots() }}><option value="" disabled>选择插槽</option>{slots.map(slot => <option key={slot.id} value={slot.id}>{slot.title}{slot.learnerEligible ? '（孩子可见）' : '（仅家长）'}</option>)}</select><button type="button" onClick={() => { const select = document.getElementById(`slot-${v.id}`) as HTMLSelectElement; if (select.value) void lifecycle(v.id, select.value) }}>绑定并发布</button></div>}</div>)}</article>) : <p className="formHint">还没有上传内容。</p>}</section>
  </section>
}
