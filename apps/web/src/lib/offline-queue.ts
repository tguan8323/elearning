...
export async function saveRecordAndQueue(
  record: LocalRecord,
...
export async function applyPulledChanges(
  changes: Array<LocalRecord & { sequence: string }>,
  cursor: string,
) {
  const db = await openOfflineDatabase();
  return new Promise<void>((resolve, reject) => {
    ...
    for (const incoming of changes) {
      ...
      request.onsuccess = () => {
        ...
        if (
          ...
          (incoming.version === local.version && incoming.deleted)
        )
...
export async function pendingOperationCount() {
  ...
  const count = await requestResult(
    db.transaction("operations").objectStore("operations").count(),
  );
  db.close();
  return count;
}
...
export async function readSyncCursor() {
  ...
  const cursor = (await requestResult(
    db.transaction("metadata").objectStore("metadata").get("cursor"),
  )) as string | undefined;
  db.close();
  return cursor ?? "0";
}
...
export async function pullChanges(fetcher: typeof fetch = fetch) {
  ...
  const response = await fetcher(
    `/api/sync/changes?cursor=${encodeURIComponent(cursor)}`,
...
export async function replayQueue(fetcher: typeof fetch = fetch) {
  ...
  for (const operation of operations.sort((a, b) =>
    ...
    if (response.status === 409) {
      ...
      db.close();
      throw new Error(`同步冲突：${JSON.stringify(detail)}`);
    }
    if (!response.ok) {
      db.close();
...
export async function resolveConflict(
  ...
  const operation = (await requestResult(
    db.transaction("operations").objectStore("operations").get(operationId),
  )) as OfflineOperation | undefined;
  if (!operation) {
    db.close();
    return;
  }
  ...
  if (choice === "server") operations.delete(operationId);
    ...
    operations.put({
      ...operation,
      baseVersion: currentVersion ?? operation.baseVersion,
    });
  await new Promise<void>((resolve, reject) => {
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error);
...
export async function readLessonPackage(): Promise<OfflineLessonPackage | null> {
  ...
  const value = (await requestResult(
    db.transaction("metadata").objectStore("metadata").get("lesson-package"),
  )) as OfflineLessonPackage | undefined;
  db.close();
  return value ?? null;
}
...
export function createOperation(
  input: Omit<OfflineOperation, "operationId" | "createdAt">,
): OfflineOperation {
  return {
    ...input,
