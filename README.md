...
初始化命令只接受交互式终端输入，密码至少 12 个字符且输入时不回显；不要把邮箱或密码写在命令参数、`.env`、源码或日志中。重复初始化不会创建第二个家长账号。项目没有公众注册入口。忘记密码时在服务器交互式终端运行 `corepack pnpm --filter @family-english/api parent:reset-password`，重置成功会立即撤销全部旧会话；同样不得通过命令参数传入凭据。
